...
# Contributed by DongHM <donghm@viettel.com.vn> 2019
...
parse_smartctl_scsi_error() {
  local disk="$1"
  local disk_type="$2"
  local labels="disk=\"${disk}\",type=\"${disk_type}\""
  while read line; do
    attr_type="$(echo "${line}" | tr '=' ':' | cut -f1 -d: | sed 's/^ \+//g' | tr ' ' '_')"
    attr_value="$(echo "${line}" | awk '{print $8}' )"
    case "${attr_type}" in
    read) read_error_value="$(echo "${attr_value}" | awk '{ printf "%e\n", $1 }')" ;;
    write) write_error_value="$(echo ${attr_value} | awk '{ printf "%e\n", $1 }')" ;;
    verify) verify_error_value="$(echo ${attr_value} | awk '{ printf "%e\n", $1 }')" ;;
    esac
  done
  [ ! -z "$read_error_value" ] && echo "read_total_uncorrected_errors{${labels}} ${read_error_value}"
  [ ! -z "$write_error_value" ] && echo "write_total_uncorrected_errors{${labels}} ${write_error_value}"
  [ ! -z "$verify_error_value" ] && echo "verify_total_uncorrected_errors{${labels}} ${verify_error_value}"
}
...
device_list="$(/usr/sbin/smartctl --scan-open | awk '/^\/dev/{print $1 "|" $3}' | grep sd)"
...
for device in ${device_list}; do
  ...
  scsi) 
    ...
    # Get the SMART error for scsi device
    # NOTE: This code is useful with the disk not in RAID mode
    /usr/sbin/smartctl -l error -d "${type}" "${disk}" | parse_smartctl_scsi_error "${disk}" "${type}"
    ;;
